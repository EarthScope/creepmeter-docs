...
- Grouped by directory where the channel's output files were physically found on disk (`.bot`/`.jl`/`.off`/`.edit`/`_results`), or — failing that — where the first 3 letters of the raw bottle name uniquely match exactly one existing directory's name (e.g. `c46v`→`c46_dir`). Left under **aux** when no directory name matches, or more than one does (e.g. `cfwv`'s `cfw` prefix matches `cfw_dir`, `cfwHR_dir`, AND `cfwLR_dir` — genuinely ambiguous, since the battery could serve any of the three pipelines) — left ungrouped rather than guessed. See [`creepmeter_calibrated.csv`](creepmeter_calibrated.csv)'s `dir_match_method` column for which rule resolved each row.
